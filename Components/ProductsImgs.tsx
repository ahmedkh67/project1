...
const ProductsImgs = () => {
  return (
    <>
    <div className="flex w-full flex-col mb-0">
      {ProductImages.map((item) => (
          <motion.div initial={{scale: 0.95}} whileHover={{scale: 1}} transition={{duration: 1.1, ease: easeInOut}} className="flex flex-col w-full justify-center items-center p-4 md:mb-10 mt-[60%] md:mt-10 sticky md:top-10 top-[40%] ">
          <Image src={item.src} width={800} height={800} alt="im" className="rounded-3xl mx-3 mb-3  md:translate-y-0 -translate-y-10" />         
        </motion.div>
      ))}
    </div>
